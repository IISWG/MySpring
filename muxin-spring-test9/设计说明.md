...
**cn.muxin.springframework.test.bean.IUserDao**
...
- 这次我们删掉 UserDao，定义一个 IUserDao 接口，之所这样做是为了通过 FactoryBean 做一个自定义对象的代理操作。

**cn.muxin.springframework.test.bean.UserService**
...
**cn.muxin.springframework.test.bean.ProxyBeanFactory**
...
![](image/image-20220622145620871.png)

- 对象后面的这一小段字符串就是16进制哈希值，在对象头哈希值存放的结果上看，也有对应的数值。只不过这个结果是倒过来的。
- 另外可以看到 44974b60@65d6b83b、44974b60@d706f19，这两个对象的结尾16进制哈希值并不一样，所以我们的原型模式是生效的。
...
- 关于 FactoryBean 的调用并没有太多不一样，因为所有的不同都已经被 spring.xml 配置进去了。当然你可以直接调用 spring.xml 配置的对象 `cn.muxin.springframework.test.bean.ProxyBeanFactory`
...
- 虽然看上去这一点实现并不复杂，甚至有点简单。但就是这样一点点核心内容的设计了，解决了所有需要和 Spring 结合的其他框架交互链接问题。*如果对此类内容感兴趣，也可以阅读小傅哥[《中间件设计和开发》](http://mp.weixin.qq.com/s?__biz=MzIxMDAwMDAxMw==&mid=2650730395&idx=1&sn=73bf265c3687ad09f17bf325b33068b8&chksm=8f610e79b816876f8522a926bd201fd3e1fb659227e307263262a6a6b368a551831f565567d4&scene=21#wechat_redirect)*
