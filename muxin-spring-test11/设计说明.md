...
![图片](image/640-16562198903042.png)
...
![图片](image/640-16562212170984.png)

- 拆解过程可以参考截图 ，我们需要把代理对象拆解出来，因为它可以是 JDK 的实现也可以是 Cglib 的处理。
...
**cn.muxin.springframework.aop.Pointcut**
...
**cn.muxin.springframework.aop.ClassFilter**
...
**cn.muxin.springframework.aop.MethodMatcher**
...
**cn.muxin.springframework.aop.AdvisedSupport**
...
**cn.muxin.springframework.aop.framework**
...
**cn.muxin.springframework.aop.framework.JdkDynamicAopProxy**
...
**cn.muxin.springframework.aop.framework.Cglib2AopProxy**
...
- 从对 Proxy#newProxyInstance、MethodInterceptor#invoke，的使用验证切面核心原理以及再把功能拆解到 Spring 框架实现中，可以看到一个貌似复杂的技术其实核心内容往往没有太多，但因为需要为了满足后续更多的扩展就需要进行职责解耦和包装，通过这样设计模式的使用，以此让调用方能更加简化，自身也可以不断按需扩展。
